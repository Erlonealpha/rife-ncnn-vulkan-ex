...
rife-ncnn-vulkan-ex uses [ncnn project](https://github.com/Tencent/ncnn) as the universal neural network inference framework.

## Additional Features
- Real-time progress display
- Stream output to stdout support
- Support for pause, resume, and graceful shutdown
- Input image caching
- Automatic breakpoint resumption
...
# Find the source fps and format with ffprobe (e.g., 24fps, AAC audio)
ffprobe input.mp4

# Decode input video as lossless JPEG image sequence
ffmpeg -i input.mp4 -f image2 -qscale:v 1 input_frames/frame_%08d.jpg

# The following two steps can use streaming output mode without intermediate frame storage (see below)
# Interpolate input image sequence, output interpolated frame sequence, default 2x output
./rife-ncnn-vulkan-ex -i input_frames -o output_frames

# Encode output interpolated frame sequence as H.265/HEVC video file (48fps)
...
### Streaming Output to FFmpeg (Real-time Video Processing)

Supports streaming output without intermediate frame storage. Use the `-r` option:

```shell
# Using raw output mode (output image as BGR24 rawvideo format)
# Note: replace 1920x1080 with your actual input video resolution (this is required!)
...
- `num-frame` = target frame count, only supported by rife-v4 and above models
- `time-step` = interpolation time step, only supported by rife-v4 and above models
...
### Interpolate with rife-anime model
...
### Interpolate with rife-anime model + TTA-s
...
- https://github.com/TNTwise/rife-ncnn-vulkan for workflow reference